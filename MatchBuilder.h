...
namespace shl {
	...
	class MatchBuilder {
		...
		public:
			...
			template<class F>
			constexpr MatchBuilder<Resolver, Args..., shl::decay_t<F>> operator|(F&& fn) { return add_case(fn); }
			...
			template<class F>
			constexpr Matcher<Resolver, Args..., shl::decay_t<F>> operator||(F&& fn) { return add_case(fn); }
